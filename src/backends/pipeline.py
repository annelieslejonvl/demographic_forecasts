...
class UnifiedPipeline:
    ...
    def tune_threshold(
        ...
        **kwargs
        ...
        Args:
            validation_data: Validation data (format depends on backend)
        ...
        """
        if not self._is_fitted:
            raise RuntimeError("Pipeline must be fitted before tuning threshold")

        if self.config.backend == BackendType.SPARK:
            # For Spark, validation_data is a DataFrame
            X_val, _ = self._prepare_data(validation_data, self.feature_cols_, fit=False)
            features_col = self.preprocessor_.get_output_col()
            X_val = X_val.select(features_col, self.config.label_col)
            y_val = X_val  # Pass the full DataFrame for Spark models
        else:
            # For PyTorch/XGBoost, validation_data is (X, y) tuple
            X_val, y_val = self._prepare_data(validation_data, self.feature_cols_, fit=False)

        return self.estimator_.tune_threshold(X_val, y_val, strategy=strategy, **kwargs)

    def get_threshold(self) -> float:
        """Get the current classification threshold."""
        if not self._is_fitted:
            raise RuntimeError("Pipeline must be fitted before getting threshold")
        return self.estimator_.get_threshold()

    def set_threshold(self, threshold: float) -> None:
        """Set the classification threshold."""
        if not self._is_fitted:
            raise RuntimeError("Pipeline must be fitted before setting threshold")
        self.estimator_.set_threshold(threshold)
